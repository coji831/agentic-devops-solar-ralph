> **SOLAR v4 Reference** — Not generated by `solar-install.prompt.md` (base install). Moved here from `.github/solar-system/schemas/` for future consideration. Describes the v4 typed payload catalog validated by `subagent-stop.cjs` and `subagent-start.cjs` — v4 hooks not present in the current 2-hook base harness. The 6 base install schemas (`scout-findings`, `dev-progress`, `review-result`, `qa-result`, `designer-output`, `implementer-handoff`) are still generated by the installer.

# Handoff Types Catalog

**Path (original):** `.github/solar-system/schemas/handoff-types.md`
