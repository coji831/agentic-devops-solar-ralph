...
> **Status:** RELEASED — v5.7.5 (2026-09-20). v5.4.0 gave the **HTTP runner full role capacity
...
>
> **v5.7.4 (2026-09-20)** closes a clock that measured the wrong thing. `duration_ms` is
> `time.time() - started_at` and `started_at` is set once per CLI INVOCATION, so a run
> driven step by step with `--result` recorded the LAST step's clock as the run's —
> **read**, the Promyro card with 3 988 completion tokens against `duration_ms: 48`.
> `node_ms` is new: every node's clock, summed through the checkpoint, so it totals across
> a resume the way `tokens_in` does. It is named for what it counts because it is **node**
> time, not wall time, and `duration_ms` is kept unchanged beside it. See §30.
>
> **v5.7.5 (2026-09-20)** bounds the one clock nobody had stated. The client a CHAT call used was
> constructed with **no timeout at all**, so a hung link was limited by the SDK's defaults alone —
> a 600 s read with two retries, which is **three** of them, because `APITimeoutError` subclasses
> `APIConnectionError`. About thirty minutes, with nothing of ours in the loop. The budget is now
> stated in constants and the worst case is one multiplication: `180 x (1 + 2) = 540 s`. The
> construction moved into `chat_client()` so a test can watch a hung endpoint fail in a second
> instead of waiting three minutes to prove the same thing. See §31.
...
## 29. A range bounds lines, not chars (2026-09-19)

The one defect in this list that could take a local window out on its own. Found while wiring RTK,
and measured rather than reasoned about: `read_file(rel, 1, 60)` returned **282,669 chars ≈ 70k
tokens** for a file whose ONLY line is 282,604 chars (a `.tsbuildinfo`). The whole-file path capped
at 40,000; the ranged path capped nothing — and its docstring says why, because it assumed a bounded
number of **lines** is a bounded number of **chars**.

| bound         | value                                                     | marker on the way out                                                       |
| :------------ | :-------------------------------------------------------- | :-------------------------------------------------------------------------- |
| one line      | `MAX_LINE_CHARS` 4,000 (== `commands.DEFAULT_MAX_OUTPUT`) | `…[line 1 elided: 282604 chars, first 4000 shown]`                          |
| the selection | `MAX_READ_CHARS` 40,000                                   | `…[elided N chars between lines X and Y; narrow the range with start/end]…` |

The range still reaches **past** the whole-file cap — that is the feature it was built for, and the
test that asserts it still passes. What changed is that neither elision is silent, which is the same
rule as the stub that reads as a success (v5.6.2) and `tokens: 0/0` (v5.6.4): **a model that cannot
see a gap will report reading what it did not.**

Measured on the same file: 282,669 → **4,113** chars (≈70,667 → ≈1,028 tokens); the whole-file path
unchanged at 40,061; a normal 60-line range byte-identical at 1,573 chars. Tests **215 → 219**.

## 30. Two clocks on one card — node time, and the writing invocation's (2026-09-20)

### The reading

Every other metric on the run-card totals correctly across a resume, because each is an
`operator.add` channel and the checkpoint carries it: `tokens_in`, `tokens_out`, `tool_calls`,
`decisions_log`. **`duration_ms` was not one of them.** It is `time.time() - started_at`;
`started_at` is set once per CLI INVOCATION (`cli.py:105`); and `_write_artifacts` rewrites the card
at every `--json` step. So on a run driven step by step with `--result` — which is how
`scripts/solar-run.py` drives a link, and how the Promyro engagement drives every one — the card
recorded the **last step's** clock and called it the run's.

**Read**, and the card said so itself. `.solar/runs/Q-2026-09-20-01-implementer.json`:

| field               |              value |
| :------------------ | -----------------: |
| `tokens.in` / `out` | 32 961 / **3 988** |
| `tool_calls`        |                 21 |
| `attempts`          |                  3 |
| `duration_ms`       |             **48** |

No hosted model emits 3 988 tokens in 48 ms. Nothing noticed, because **no test in this repo named
`duration_ms` at all** — the field was written on every run and asserted on none.

### The fix, and the name it forced

`node_ms` is new: `graph._timed` wraps every node and adds that node's clock to an `operator.add`
channel, so the checkpoint accumulates it the way it accumulates tokens. Every node is timed, not
only `specialist` — a node set timed unevenly gives a number that drifts when the graph changes
shape. A node that calls `interrupt()` returns through nothing, so it adds nothing, which is right
rather than convenient: LangGraph re-runs that node from the top on resume, and counting the
interrupted pass as well would bill one node's work twice.

**The first name was `elapsed_ms`, and a measurement killed it.** A real stub run recorded:
...
The gap is everything outside a node — opening the checkpoint, compiling the graph — and for a stub
that is the _whole_ cost of the run. So `elapsed_ms` would have promised wall time and delivered node
time, and two fields that both read as "duration" invite comparing them. Renamed for what it counts;
v5.7.3's rule (`passed` → `approved`) applied to a clock.

`duration_ms` is **kept and unchanged**. It is a real reading of a different thing, and redefining it
would silently rewrite the meaning of every card already on disk. This release is additive: a patch,
not a migration.

`node_ms` is `null` when no node was timed — a state built by hand, or a driver's error row — because
`0` and "never timed" would be the same digits. That is `tokens.reported`'s asymmetry, applied to a
clock.

### The guard it needed

`node_ms` is a reducer, so it inherits across a re-used thread exactly as `tokens_in` does. TD-5.6-6's
fresh-start reset has to cover it, and §24's test listed the channels that existed when it was written.
It now lists this one.

### Measured

`graph.py` reads `time` nowhere else, so the module can be replaced on the graph alone — which makes
the millisecond assertable exactly, and this rule is not assertable any other way.
...
Three nodes timed, and the interrupting fourth added nothing: that is the load-bearing number, and it
is what makes "a resume cannot bill one node twice" a measurement rather than a hope.

Tests **257 → 261**. Found while building the Promyro engagement's tuning view (`render-runs.py`),
whose whole job is to print `n/a` rather than a number that means "not measured" — a view built to
refuse a `0` is what noticed a clock that could not cover its own run.

---

## 31. The chat client had no clock at all — a hung link cost thirty minutes (2026-09-20)

### The reading

`executor.py` built the client a CHAT call uses with **no `timeout` argument**:
...
`known_models`, three hundred lines above it, passed one — `timeout=15.0`. So this was not a policy
about timeouts; it was an absence. The bound was the SDK's: a **600 s read with `max_retries=2`**,
and because `APITimeoutError` subclasses `APIConnectionError` the SDK **retries a timeout**, so those
are three attempts. The honest answer to "what does a hung link cost" was **about thirty minutes,
with nothing of ours in the loop**.

**Measured rather than inferred, and by a wiretap.** The Promyro engagement's T1.1 built a fake
provider to see what the `http` path actually sends, and it recorded **3 identical requests, 2 196 B
each** against a card that claimed one attempt. That is the reading; this release is the one that owns
it, four days later.

### The fix, and why it is a function

The budget is **stated**, in constants above the client, and the worst case is one line:

    read x (1 + retries) = 180 x 3 = 540 s = 9 minutes, against 1 800 s.

`connect` is deliberately much shorter than `read`. An unreachable or black-holed host is the case
that _looks_ like a hang while nothing is being served, and ten seconds is enough for a host that is
going to answer at all. `read` stays generous because it bounds a model **thinking**, not a transfer:
the slowest **whole run** measured on this harness is 21.5 s (T2.3), so 180 is an order of magnitude
of headroom before a real run is failed rather than hung.

**The construction moved into `chat_client()` for a reason that is about testing.** A budget stated
inline is asserted by a reader noticing an absence; a budget in a factory is asserted by a test, and a
test can pass a short `read_timeout` to watch a hung endpoint fail in a second rather than wait three
minutes to prove the same thing. `Timeout` is imported from `openai` rather than `httpx` — they are
the same class, `openai.Timeout is httpx.Timeout` — so this adds no dependency.

### Measured

Three tests in `test_providers.py`, whose subject is already _what a real server receives_ — and a
server that never answers is a thing a real server does.

| test                                            | what it pins                                                                          |
| :---------------------------------------------- | :------------------------------------------------------------------------------------ |
| `test_the_chat_client_states_its_budget`        | the read, connect and retry values **reach the client**, and `180 x 3 == 540`         |
| `test_a_hung_endpoint_fails_instead_of_hanging` | a server that accepts and sleeps: `APITimeoutError` in **1.88 s** at `read_timeout=1` |
| `test_the_run_path_gets_the_stated_budget`      | the run path **constructs through the factory** and overrides nothing                 |

The third is this repo's own rule applied to itself: §27's warning was that a unit test on a value
computed correctly does not catch a value computed and then **not used**, so the run path is spied on
rather than trusted. Its `hung_endpoint` is a **threading** server deliberately — `HTTPServer`
handles one request at a time, so `shutdown()` would wait out the handler's sleep and the test would
take thirty seconds to tear down.
