...
> **Status:** RELEASED — v5.3.0 (2026-09-05). Runtime (`solar-governor/`)
> done; mandarin pilot (`solar-v5-wire`, reference only) T1–T5 PASS +
> epic-25 Phase A delivered + driver-orchestrated verify close-out APPROVED;
> eval battery 18/18. Full epic-25 with the detailed pipeline = next, on
> `main` after installing v5.3.0.
> **Release:** v5.3.0 (2026-09-05)
...
`SPECIALISTS` = `{ role: { system, tools, next_edges, model } }` — the graph equivalent of `AGENTS.md` §3.
...
| Hooks / write-guard | none                                        | workspace MCP guard                  |
| Model               | single fast model                           | tier/model per node                  |
...
| :--------- | :------------------------------------------------------------------------------ | :------------------------------------------------------------------------------------ |
| Engine CLI | `uvx solar-governor init --profile light\|full` · `solar-governor run "<task>"` | `uv` = single binary, no venv, CI-friendly; writes `.solar/config.yaml` + `.mcp.json` |
| npm shim   | `npx solar-governor init`                                                       | thin wrapper for JS/TS repos                                                          |
| Container  | `ghcr.io/solar-ralph/governor`                                                  | headless cloud runs; state → volume/GCS/S3                                            |
| CI action  | `solar-ralph/governor-action` (`profile:`)                                      | install + run in CI, zero local setup                                                 |

**Cloud-ready =** headless (`solar run` in bare terminal/CI/container) · non-interactive (`--ci`/`--yes`, env `SOLAR_PROFILE`, `SOLAR_APPROVAL=off`) · durable state (volume/object store) · remote later (HTTP/SSE gateway).
...
- **Operational:** per-run telemetry (checkpoint written, ledger rendered, traces) → repo-local events · rolling eval metrics (completion %, first-pass APPROVED %, rework count, tokens/task) · periodic `doctor` (MCP alive, checkpointer durable, model fallback) · read-only guard node denies writes outside the repo.
...
1. Install governor runtime (`uvx solar-governor init --profile <light|full>`).
...
- **Timeline:** (a) v5 full migration — `SPECIALISTS` registry + compactor + light/full profiles + install CLI (`uvx`/`npx solar-governor init`); (b) **pilot: v5 LIGHT on mandarin for 1 epic** (real repo, performance gate — §14.7); (c) first real deployment = the master repo (resume-kb) as dogfooding; (d) hub router-as-graph (R9) + IDE-agnostic surfaces (CLI/HTTP-SSE).
