...
> **Status:** RELEASED — v5.6.3 (2026-09-19). v5.4.0 gave the **HTTP runner full role capacity
...
The HTTP runner had been *shipped* many times and *proven* rarely: the provider key lived
...
`sqlite3.OperationalError` and exit 1, while the interactive path on the *same repo* worked
...
provider" and neither was visible by reading the runner: `run --json` on a clone *looked* like
it would work, and a config *looked* correct. The first probe also failed to detect them for
