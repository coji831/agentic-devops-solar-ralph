...
> **Status:** RELEASED — v5.7.0 (2026-09-19). v5.4.0 gave the **HTTP runner full role capacity
...
The OpenAI SDK insists on a non-empty `api_key` string; the *servers* do not want one. Every
...
`/chat/completions` but expose no model list, so `doctor` says *unverified* rather than
