...
3. EXECUTE LOOP [for each stage in sequence]:
   ...
   3b. CONTEXT — dispatch Context Summarizer before each specialist:
   ...
   Await `verification-artifacts/{task-id}-digest.json`.
   Read the digest — it is compact (~20 lines). Include key facts, refs, and warnings inline in the next dispatch prompt.

   3c. EXECUTE — dispatch specialist:
   Dispatch prompt contains: task description + context digest (inline) + SKILL.md path + result path + return instruction.
   Specialists do NOT have the `read` tool — they receive all context through the digest you pass inline.
   Re-dispatches (remediation, re-test, re-audit) MUST explicitly repeat all items — each subagent is a stateless isolated session with no memory of prior dispatches.
...
5. Context summarizer discipline: follow `context-summarizer.instructions.md` for the dispatch pattern. Key rule: read the digest JSON and include key facts inline in the specialist dispatch prompt — specialists cannot read files directly.

6. Artifact discipline:
   - Artifact schemas are sparse: `{task_id, refs[]}` plus stage-specific fields only.
   - Substantive content (findings, reasons, summaries) goes to the Decisions Log, not the artifact.
   - Artifacts are reference indices — readers use `refs[]` to know where to look if they need more detail.

7. Context discipline: use `startLine/endLine` for all file reads. Write findings to `verification-artifacts/` instead of accumulating raw content in context. For AGENTS.md: read Section Index first (lines 1–15), then read only the section needed. After every 3 specialist dispatches, perform a context roll-up: summarize completed stages into a single Decisions Log entry and clear any stale file contents from working memory.

8. Operation failure protocol (applies to all tool calls): on any tool call that returns an error or partial failure: (a) retry once using an alternative form (e.g. `replace_string_in_file` instead of `multi_replace_string_in_file`, smaller range); (b) if retry fails: append to Decisions Log as `BLOCKED: <tool-name> failed — <one-line reason>. Pending retry on next pickup.`; (c) do NOT silently continue past a failed write to `.github/.ai_ledger.md`; (d) for non-ledger failures (e.g. `create_file` on existing file): retry, then proceed if content is already correct.
