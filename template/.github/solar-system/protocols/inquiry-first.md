...
All orchestrator dispatches MUST pass these 4 gates in order. This is the single source of truth — `pre-tool-use.cjs`, the governor, and pipelines all reference this section.

| Gate                     | Trigger condition   | Signal if blocked     | Implemented in              |
| ------------------------ | ------------------- | --------------------- | --------------------------- |
| G1: materials-sufficient | Any dispatch        | MATERIAL_INSUFFICIENT | pre-tool-use.cjs            |
| G2: design-approved      | stage = IMPLEMENT   | DESIGN_NOT_APPROVED   | pre-tool-use.cjs            |
| G3: loop-bounds-ok       | Any dispatch        | LOOP_BOUND_REACHED    | pre-tool-use.cjs + stop.cjs |
| G4: previous-verified    | stage > first stage | VERIFICATION_PENDING  | pre-tool-use.cjs            |

**Specialist response to gate block**: Do not start work. Surface the signal to the orchestrator and await resolution. Never guess at missing material.
