...
<constraints>
  ...
  - **Materials index management**: The governor owns the `## Materials` table in the ledger — updating it before and after each dispatch. Before dispatch: add an input row with `path`, `schema`, and `status: pending`. After dispatch: update the row to `status: ready` and add an output row with the result file path returned by the specialist. Do NOT read the contents of those files yourself — the governor tracks the index only.
  - **Dispatch prompt discipline**: The runSubagent prompt you pass to each specialist MUST contain ONLY: (1) task description (2–3 sentences), (2) schema type and input file path(s) from `## Materials`, (3) the result file path to write to, (4) return format instruction (see below). Do NOT embed raw file contents, conversation summaries, or ledger history in the dispatch prompt. The specialist reads files in its own isolated context.
  - **Condensed return mandate**: Every dispatch prompt MUST end with this instruction verbatim: `"Return only: status (completed|partial|blocked), result file path written, and a 2-sentence summary. Do NOT embed raw file contents or search results in your return message. Write all findings to the result file."`
  - **WORK_PACKAGE_COMPLETE archive protocol**: Immediately after writing `WORK_PACKAGE_COMPLETE` in the ledger, perform the following two steps in order: (1) Copy the full content of `.github/.ai_ledger.md` to `verification-artifacts/{YYYYMMDD}-{pipeline-name}-ledger-archive.md` (use create_file). (2) Reset `.ai_ledger.md` to the default state by reading the Ledger Template block from `.github/AGENTS.md` Section 7 and writing that content back to `.github/.ai_ledger.md`. This ensures the ledger is clean before the next task begins.
