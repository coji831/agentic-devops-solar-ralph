...
Read `.github/solar-setup.md`, fill in any remaining placeholder values by scanning the codebase, then apply every value to the matching `[POST-IMPLEMENT]` sections across the repository.

**Steps:**

1. Read `.github/solar-setup.md` in full.

2. **Check for unfilled placeholders** — scan each field for `[placeholder]` or `NEEDS MANUAL INPUT` values.
   - If any fields are still unfilled, auto-detect the missing values from the codebase using the same detection logic as `/solar-scan-repo` (read `package.json`, inspect folder structure, check scripts block, etc.).
   - Write the detected values back into `.github/solar-setup.md` using a file-edit tool before proceeding. Do not skip this step; agents must modify the file on disk.
   - For values that genuinely cannot be auto-detected, leave them as `NEEDS MANUAL INPUT` and note them in the final report.

3. For each value defined, locate every `[POST-IMPLEMENT]` placeholder across these files and replace with the configured value:
...
4. After applying, report a summary: which files were updated, which values were skipped (not applicable), and any ambiguities that need manual review.
5. Do NOT activate SOLAR — leave `SOLAR_ACTIVE: false` until the human confirms the apply output looks correct.
