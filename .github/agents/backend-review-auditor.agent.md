...
- `crossDomainAlignmentChecked`: `true` | `false` (required when API contract changes are present)
  </tier_definition>
...
4. **Cross-Domain Alignment Check** — If the change touches any API endpoint, response schema, or shared type:
   - Read the Cross-Domain Dependencies section from the design doc in `## Handoff Payload`
   - Verify the backend implementation matches the agreed API contract (paths, request/response shapes, HTTP methods)
   - If no Cross-Domain Dependencies section exists but the change touches public API endpoints: flag as `CROSSDOMAIN_UNVERIFIED` in findings (severity: HIGH)
5. Identify missing tests or unsafe assumptions.
...
**Produces**: `verification-artifacts/{task-id}-review-result.md` conforming to `review-result.schema.json`
